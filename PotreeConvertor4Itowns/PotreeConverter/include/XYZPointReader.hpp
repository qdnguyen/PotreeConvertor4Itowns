...
public:
	XYZPointReader(string file, string format, vector<double> colorRange, vector<double> intensityRange, string projection)
	: stream(file, std::ios::in | std::ios::binary)
	{
		...
		while(readNextPoint(projection)){
			Point p = getPoint();
			aabb.update(p.position);
			pointCount++;
		}
        
        //start fro begining, xyz has not header
		stream.clear();
		stream.seekg(0, stream.beg);
	}
		...
		while(getline(stream, line)){
			...
			for(const auto &f : format) {
				...
				if(f == 'x'){
                    ...
                    b = 0;//(unsigned char)(255.0f * (stof(token) - colorOffset) / colorScale);
				}else if(f == 'i'){
					intensity = (unsigned short)( 65535 * (stof(token) - intensityOffset) / intensityScale);
				}else if(f == 's'){
					// skip
            ...
            if(projection == "LAMBERT2WGS84"){
                ...
                projector.carthographicToCartesian(position);
            }
