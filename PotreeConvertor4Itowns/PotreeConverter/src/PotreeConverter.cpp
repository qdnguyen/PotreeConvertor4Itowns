...
PointReader *PotreeConverter::createPointReader(string path, PointAttributes pointAttributes, string projection){
	...
	if(boost::iends_with(path, ".las") || boost::iends_with(path, ".laz")){
		...
		reader = new XYZPointReader(path, format, colorRange, intensityRange, projection);
...
AABB PotreeConverter::calculateAABB(){
	...
	if(aabbValues.size() == 6){
		...
		for(string source : sources){
			...
			PointReader *reader = createPointReader(source, pointAttributes, projection);
			
			AABB lAABB = reader->getAABB(projection);
